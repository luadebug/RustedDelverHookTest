...
pub fn get_jni_get_created_jvms() -> Option<GetCreatedJavaVMs> {
    let jvm_module = HINSTANCE::GetModuleHandle(Option::from("jvm.dll"));
    let jvm_proc_address = jvm_module.unwrap().
        GetProcAddress("JNI_GetCreatedJavaVMs").
        expect("Did not find proc_nme JNI_GetCreatedJavaVMs"); //, 
